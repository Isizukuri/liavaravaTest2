...
class TestWidgetPage(TestCase):
    ...
    def test_widget_page(self):
        ...
        #Checking if there is an element with help-text in rendered template to know, if it is properly rendered
        self.assertInHTML(
        ...
        #It is impossible to do it in other way, because test client doesn`t give host url
...
class TestWidgetReturn(TestCase, SimpleTestCase):
    ...
    def test_widget_page(self):
        ...
        self.assertIn(unicode(response.content.split('<div>')[1].split('</div>')[0]), notes)
